...
#
# The Python pipeline runs FIRST: the front end imports an artifact the pipeline
# writes (web/src/lib/data/skeleton.json), which is gitignored and regenerated,
# never committed. A build that skipped this step would either fail at import or
# ship yesterday's numbers — the latter is the outcome PLAN.md forbids for a
# tool whose premise is numeric credibility.
#
...
echo "[1/2] pipeline: computing artifacts (Python)"
python -m incrementality.build_skeleton

echo "[2/2] front end: static build (SvelteKit adapter-static)"
